...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(  
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-lg bg-transparent px-4 py-6 lg:flex dark:bg-transparent",
    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-neutral-300 dark:text-neutral-300 font-semibold"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
...
export const MobileNavToggle = ({
  ...
  return isOpen ? (
    <IconX className="text-white dark:text-white w-10 h-10" onClick={onClick} />
  ) : (
    <IconMenu2 className="text-white dark:text-white w-10 h-10" onClick={onClick} />
  );
};
